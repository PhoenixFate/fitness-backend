...
public class AdminActionController {
    ...
    public ResponseEntity<R> list(@RequestParam Map<String, Object> params){
        PageUtils page = actionService.queryPage(params,new UserEntity());
        return ResponseEntity.ok(new R(page));
    }

    @GetMapping("/type")
    @ApiOperation("已经分好类别的动作列表")
    public ResponseEntity<R> listWithType(@RequestParam Map<String, Object> params){
        List<ActionTypeWithActionsDto> actionTypeWithActions = actionService.getActionsWithType(params,new UserEntity());
    ...
    @PostMapping()
    @ApiOperation("新增动作")
    public ResponseEntity<R> save(@RequestBody @Valid ActionWithAllDto action){
